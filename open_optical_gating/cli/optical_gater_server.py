...
class OpticalGater:
    ...
    def analyze_pixelarray(self, pixelArray):
        ...
        time_fin = time.time()
        pixelArray.metadata["processing_rate_fps"] = 1 / (time_fin - time_init)
    ...
    def determine_state(self, pixelArray, modeString="determine period"):
        ...
        if self.ref_frames is not None:
            ...
            ref.save_period(
                self.ref_frames,
                metadata={
                    "period": self.pog_settings["reference_period"],
                    "drift": self.pog_settings["drift"],
                },
                parent_dir=self.settings["period_dir"],
            )
